...
```bash
cp -r downstream_examples/template downstream_examples/your_task
```
...
```bash
python -m downstream_examples.your_task.3_finetune_template_1D --deterministic warn
```
...
CUDA_VISIBLE_DEVICES=0,1 python -m downstream_examples.your_task.3_finetune_template_1D

# Quick sanity check
CUDA_VISIBLE_DEVICES=0 python -m downstream_examples.your_task.3_finetune_template_1D \
